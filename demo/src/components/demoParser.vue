...
<template>
  <main class="max-w-7xl mx-auto"><h1 class="text-center text-2xl font-semibold my-10">Graph builder</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      <!-- output -->
      <div>
        <div id="root" class="bg-white rounded p-3 border"></div>

        <div v-show="showAnimation" class="flex gap-3 mt-3">

          <button class="bg-white hover:bg-gray-200 transition-colors border rounded px-4 py-2"
                  @click="animation('start')">start
          </button>
          <button class="bg-white hover:bg-gray-200 transition-colors border rounded px-4 py-2"
                  @click="animation('pause')">pause
          </button>
          <button class="bg-white hover:bg-gray-200 transition-colors border rounded px-4 py-2"
                  @click="animation('continue')">continue
          </button>
          <button class="bg-white hover:bg-gray-200 transition-colors border rounded px-4 py-2"
                  @click="animation('stop')">stop
          </button>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <details class="border rounded p-3 bg-white">
        <summary class="my-5 text-xl">List of all commands</summary>
        <div ref="output" class="w-full grid grid-cols-1 md:grid-cols-2 gap-3">
          <div
              v-for="doc in PiDraw.documentation()"
      ...
      <details class="border rounded p-3 bg-white mt-3" open>
        <summary class="my-5 text-xl">Code examples</summary>
        <div ref="examples" class="w-full grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          >
            <h2 class="font-semibold text-lg bg-gray-200 p-3">{{ example.name }}</h2>
